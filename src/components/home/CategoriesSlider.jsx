...
export default function CategoriesSliderSection() {
  ...
  // 3. Create two separate refs for mobile and desktop sliders
  const mobileSwiperRef = useRef(null);
  const desktopSwiperRef = useRef(null);

  // Safely trigger navigation on whichever slider is currently visible
  const handlePrev = () => {
    ...
    <section className="bg-[#232321] pt-16 md:pt-24 pb-0 overflow-hidden mt-12 md:mt-24">
      <div className="container relative">
        ...
        <div className="flex justify-between items-center mb-10">
          ...
          {/* Custom Navigation Buttons (Updated to use our new handler functions) */}
          <div className="flex gap-3">
            <button 
              onClick={handlePrev}
              className="bg-[#3A3A3A] hover:bg-white text-white hover:text-[#232321] p-3 rounded-lg transition-colors flex items-center justify-center"
        ...
        {!isLoading && !error && categories.length > 0 && (
          <>
            {/* =========================================
                MOBILE SLIDER: 2 Rows Layout
                Visible only on small screens (md:hidden)
                ========================================= */}
            <div className="md:hidden w-[calc(50%+50vw)]"> 
              <Swiper
                onBeforeInit={(swiper) => {
            ...
            {/* =========================================
                DESKTOP SLIDER: 1 Row Layout
                Visible only on larger screens (hidden md:block)
                ========================================= */}
            <div className="hidden md:block w-[calc(50%+50vw)]"> 
              <Swiper
                onBeforeInit={(swiper) => {
